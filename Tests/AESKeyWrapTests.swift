...
class AESKeyWrapTests: XCTestCase {
    func testA128KW() {
        ...
        let ciphertext = try! AESCrypt.wrap(rawKey: rawKey, keyEncryptionKey: kek, algorithm: .A128KW)

        XCTAssertEqual(ciphertext, expectedCiphertext)

        let unwrappedKey = try! AESCrypt.unwrap(wrappedKey: ciphertext, keyEncryptionKey: kek, algorithm: .A128KW)
    ...
    func testA192KW() {
        ...
        let ciphertext = try! AESCrypt.wrap(rawKey: rawKey, keyEncryptionKey: kek, algorithm: .A192KW)

        XCTAssertEqual(ciphertext, expectedCiphertext)

        let unwrappedKey = try! AESCrypt.unwrap(wrappedKey: ciphertext, keyEncryptionKey: kek, algorithm: .A192KW)
    ...
    func testA256KW() {
        ...
        let ciphertext = try! AESCrypt.wrap(rawKey: rawKey, keyEncryptionKey: kek, algorithm: .A256KW)

        XCTAssertEqual(ciphertext, expectedCiphertext)

        let unwrappedKey = try! AESCrypt.unwrap(wrappedKey: ciphertext, keyEncryptionKey: kek, algorithm: .A256KW)
    ...
    func testKeyWrapWithTooLargeKey() throws {
        ...
        XCTAssertThrowsError(
            try AESCrypt.wrap(rawKey: rawKey, keyEncryptionKey: tooLargeKeyEncryptionKey, algorithm: .A128KW),
            "Invalid keysize"
        ) { error in
    ...
    func testKeyUnwrapWithTooLargeKey() throws {
        ...
        XCTAssertThrowsError(
            try AESCrypt.unwrap(wrappedKey: wrappedKey, keyEncryptionKey: tooLargeKeyEncryptionKey, algorithm: .A128KW),
            "Invalid keysize"
        ) { error in
            XCTAssertEqual(error as! AESError, AESError.keyLengthNotSatisfied)
        }
    ...
    func testKeyUnwrapWithTooSmallKey() throws {
        ...
        XCTAssertThrowsError(
            try AESCrypt.unwrap(wrappedKey: wrappedKey, keyEncryptionKey: tooSmallKeyEncryptionKey, algorithm: .A128KW),
            "Invalid keysize"
        ) { error in
            XCTAssertEqual(error as! AESError, AESError.keyLengthNotSatisfied)
        }
    ...
    func testKeyWrapEmptyKey() throws {
        ...
        XCTAssertThrowsError(
            try AESCrypt.wrap(rawKey: rawKey, keyEncryptionKey: keyEncryptionKey, algorithm: .A128KW),
            "Invalid keysize"
        ) { error in
    ...
    func testKeyUnwrapEmptyKey() throws {
        ...
        XCTAssertThrowsError(
            try AESCrypt.unwrap(wrappedKey: wrappedKey, keyEncryptionKey: keyEncryptionKey, algorithm: .A128KW),
            "Invalid keysize"
        ) { error in
